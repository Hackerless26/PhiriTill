...
// netlify/functions/branch-upsert.ts
var branch_upsert_exports = {};
__export(branch_upsert_exports, {
  handler: () => handler
});
...
var handler = async (event) => {
  try {
    ...
    if (payload.id) {
      const { error: error2 } = await supabaseService.from("branches").update({
        name,
    ...
    const { data, error } = await supabaseService.from("branches").insert({
      name,
      is_default: payload.is_default ?? false
    }).select("id").single();
    if (error) {
...
//# sourceMappingURL=branch-upsert.js.map
