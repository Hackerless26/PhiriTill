...
// netlify/functions/supplier-upsert.ts
var supplier_upsert_exports = {};
__export(supplier_upsert_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(supplier_upsert_exports);
...
var handler = async (event) => {
  try {
    ...
    if (payload.id) {
      const { error: error2 } = await supabaseService.from("suppliers").update({
        name,
        phone: payload.phone ?? null,
        email: payload.email ?? null
      }).eq("id", payload.id);
      if (error2) {
        return jsonResponse(400, { error: error2.message });
      }
      return jsonResponse(200, { status: "ok", supplier_id: payload.id });
    }
    const { data, error } = await supabaseService.from("suppliers").insert({
      name,
      phone: payload.phone ?? null,
      email: payload.email ?? null
    }).select("id").single();
...
//# sourceMappingURL=supplier-upsert.js.map
